...
# You'll have to edit rebuild.sh manually to create this base image the first time
docker build -f ./container/Dockerfile-Update --no-cache -t localdev/mrplotter:latest .

# Remove all the currently running docker containres based on the localdev/mrplotter image
